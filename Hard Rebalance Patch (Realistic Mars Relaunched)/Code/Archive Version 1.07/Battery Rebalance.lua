...
local BatteryDescOn = {
    AtomicBattery = T{"Stores Power. High capacity and max output, but charges slowly. Conversion efficiency 85%."},
    Battery_WaterFuelCell = T{"Stores Power. Amount of Power supplied is limited by the battery's max output. Conversion efficiency 90%."},
}
...
local function EffName(base, eff)
    return Untranslated(string.format("%s %d%% Efficiency", base, eff))
end
...
local function ApplyTo(obj, id)
    ...
    obj.base_conversion_efficiency = eff
    obj.display_name = name
    obj.display_name_pl = name
