...
local function OpenEarthFactions()
    if rawget(_G, "OpenElections") then
        OpenElections(nil, "earthfactions")
        return
    end
    print("RMR poi earth open miss OpenElections")
end

local function InjectEarthFactionsButton(root)
    local dlg = GetDialog("PlanetaryView") or root
    if not dlg then
        print("RMR poi earth btn miss dlg")
        return
    end
    if EarthActionAdded == dlg then
        return
    end
    if dlg.ActionById and dlg:ActionById("RMR_EarthFactions") then
        EarthActionAdded = dlg
        return
    end
    XAction:new({
        ActionId = "RMR_EarthFactions",
        ActionName = T{"EARTH FACTIONS"},
        ActionToolbar = "ActionBar",
        OnAction = OpenEarthFactions,
    }, dlg, dlg.context)
    EarthActionAdded = dlg
    if dlg.UpdateActionViews and dlg.idActionBar then
        dlg:UpdateActionViews(dlg.idActionBar)
    end
    print("RMR poi earth action", dlg.class)
end
...
local function WrapClass(name)
    ...
    if cls.Open and not Wrapped[name .. ".Open"] then
        ...
        function cls:Open(...)
            local result = old(self, ...)
            EarthActionAdded = nil
            PatchPOIPanel(self)
            RenameRecentActions(self)
            InjectEarthFactionsButton(self)
            return result
        end
