...
local RMR_DESC = {
    CaptureIceAsteroids = T{0, "Seven engineers fly into the belt and bolt engines onto several ice asteroids. Mission Control will have the choice on the last correction burn.\n\n<effect>\nScatter: <resource(n5, res)>\nMars: <resource(n3, res)>, large water deposit\nAsteroid: <resource(n4, res)>, small water deposit", n5 = 5000, n3 = 3000, n4 = 4000, res = "Water" .. (const.TerraformingParamSuffix or "T")},
    ImportGreenhouseGases = Untranslated("Three engineers take a refurbished rocket to Earth, fill it with greenhouse gases and dump the load into the Martian sky.\n\n<em>This action will cause Toxic Rains (even if Toxic Rains no longer occur due to terraforming) that are harmful to soil quality and plant life.</em>"),
    CloudSeeding = Untranslated("Three scientists seed the thin Martian clouds so rain can fall where Temperature and Atmosphere already allow liquid water. Exotic minerals go into the dispersal payload."),
    SeedVegetation = Untranslated("Three botanists haul a vast seed stock and spread cyanophytes, algae, lichens and hardier plants across the surface. The colony must grow the seeds first."),
    MeltThePolarCaps = Untranslated("Carry a nuclear package to the poles and melt the caps. Each successive strike has less effect on Water and Atmosphere.\n\n<em>Stops ongoing Cold Waves. Causes a prolonged Dust Storm even if Dust Storms no longer occur due to terraforming. Consumes the rocket.</em>"),
    LaunchSpaceMirror = Untranslated("Launch an L1 space mirror to throw more sunlight onto Mars. Raises Temperature and the output of Solar Panels."),
    LaunchSpaceSunshade = Untranslated("Launch an L1 magnetic shield against the solar wind and slow the loss of Atmosphere."),
    RepairSpaceMirror = Untranslated("Six engineers fly to L1 and set a damaged space mirror true again."),
    RepairSpaceSunshade = Untranslated("Six engineers fly to L1 and bring a magnetic shield back online."),
}
...
local function InjectCrewRequest(rocket, cargo_request)
    ...
    local id = loc and (loc.project_id or loc.id)
...
local function WrappedSetFlightData(self, departure_loc, arrival_loc)
    ...
    local id = loc and loc.project_id
    local crew = ToggleTF and id and RMR_CREW[id]
...
local function WrappedSetUIProjectParams(self)
    ...
    local crew = ToggleTF and spot and spot.project_id and RMR_CREW[spot.project_id]
    ...
    print("RMR ui crew", spot.project_id, crew.n, crew.spec)
    return result
end
...
local function RefreshSpots()
    ...
    for _, spot in ipairs(MarsScreenLandingSpots or empty_table) do
        if spot.spot_type == "project" and RMR_COST[spot.project_id] then
            local p = presets and presets[spot.project_id]
            if p then
