...
local function SetTechText(id, name, desc)
    ...
    if TechDef and TechDef[id] then
        list[#list + 1] = TechDef[id]
    end
    if Techs and Techs[id] and Techs[id] ~= list[1] then
        list[#list + 1] = Techs[id]
    end
    if ForEachPreset then
        ForEachPreset("Tech", function(tech)
            if tech and tech.id == id then
                list[#list + 1] = tech
            end
        end)
    end
    ...
    local dlg = GetDialog("XTechTree")
    local map = dlg and dlg.idMap
    local tech = Techs and Techs[id] or TechDef and TechDef[id]
    local win = map and tech and map.PresetWindows and map.PresetWindows[tech]
    if win and win.OnContextUpdate then
        win:OnContextUpdate(win.context or tech)
    end
end
