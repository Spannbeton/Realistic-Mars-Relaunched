...
local function ApplyTFTexts(cls_name)
    ...
    elseif cls_name == "CoreHeatConvector" then
        ...
        SetTechText("CoreHeatConvertor", heat_name,
            T{"New Building: <em>Central Heating System</em> (<buildinginfo('CoreHeatConvector')>) - unique heating network for Heaters and Domes. Requires Engineers. Slightly improves Temperature <icon_TemperatureTP_alt>.\n\n<grey>\"If the world seems cold to you, kindle fires to warm it.\" \n<right>Lucy Larcom</grey><left>"})
...
local function MakeTFWorkplace(cls_name, template_id, workers, spec)
    local cls = rawget(_G, cls_name)
    local wp = rawget(_G, "Workplace")
...
local function EnsureHeatMod(bld, prop, pct)
    if not bld.modifications then
        return false
    end
    local mods = bld.modifications[prop]
    if type(mods) ~= "table" then
        mods = { amount = 0, min = 0, percent = 100 }
        bld.modifications[prop] = mods
    end
    local entry
    for _, e in pairs(mods) do
        if type(e) == "table" and e.upgrade_id == Heat_UPG and e.prop == prop then
            entry = e
            break
        end
    end
    ...
    local sum = 0
    for _, e in pairs(mods) do
        if type(e) == "table" and type(e.percent) == "number" then
            sum = sum + e.percent
        end
    end
    mods.percent = 100 + sum
    return true
end

local function WriteModPercent(bld, prop, pct)
    ...
    patch_list(bld.upgrade_modifiers and bld.upgrade_modifiers[Heat_UPG])
    patch_list(bld.upgrade_id_to_modifiers and bld.upgrade_id_to_modifiers[Heat_UPG])
    return EnsureHeatMod(bld, prop, pct)
end

local function ResetProp(bld, prop)
    if not bld or not bld.SetBase then
        return
    end
    if IsValid and not IsValid(bld) then
        return
    end
    if bld.HasMember and not bld:HasMember(prop) then
        return
    end
    local base = bld["base_" .. prop]
    if type(base) ~= "number" then
        return
    end
    bld:SetBase(prop, base)
end

local function EnsureHeatUpgrade(bld, with_water)
    if not bld then
        return
    end
    bld.upgrade1_id = Heat_UPG
    bld.upgrade1_display_name = Untranslated("Central Heating")
    bld.upgrade1_description = Untranslated("Supplied by the Central Heating System.")
    bld.upgrade1_mod_prop_id_1 = "electricity_consumption"
    bld.upgrade1_mul_value_1 = -0
    if with_water then
        bld.upgrade1_mod_prop_id_2 = "water_consumption"
        bld.upgrade1_mul_value_2 = 0
    end
end
...
local function SetHeatMul(bld, pct, with_water)
    ...
    EnsureHeatUpgrade(bld, with_water)
    bld.upgrade1_mul_value_1 = -pct
    if with_water then
        bld.upgrade1_mul_value_2 = -pct
    end
    WriteModPercent(bld, "electricity_consumption", pct)
    if with_water then
        WriteModPercent(bld, "water_consumption", pct)
    end
    ResetProp(bld, "electricity_consumption")
    if with_water then
        ResetProp(bld, "water_consumption")
    end
end
...
local function ModifyHeatConvector()
    ...
    MakeTFWorkplace("CoreHeatConvector", "CoreHeatConvector", 8, "engineer")
...
local function ModifyMagneticShield()
    ...
    MakeTFWorkplace(name, name, 4, "scientist")
    for _, bld in ipairs(MapLabel(name)) do
...
local function ModifyForestation()
    MakeTFWorkplace("ForestationPlant", "ForestationPlant", 1, "botanist")
    ApplyTFTexts("ForestationPlant")
end

local function ModifyGHGFactory()
    MakeTFWorkplace("GHGFactory", "GHGFactory", 2, "engineer")
    ApplyTFTexts("GHGFactory")
end

local function ModifyCarbonateProcessor()
    MakeTFWorkplace("CarbonateProcessor", "CarbonateProcessor", 6, "geologist")
...
function OnMsg.ClassesBuilt()
    ...
    MakeTFWorkplace("CarbonateProcessor", "CarbonateProcessor", 6, "geologist")
    MakeTFWorkplace("CoreHeatConvector", "CoreHeatConvector", 8, "engineer")
    MakeTFWorkplace("MagneticFieldGenerator", "MagneticFieldGenerator", 4, "scientist")
end
