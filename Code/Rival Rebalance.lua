...
local function LockTraits()
	...
	for _, id in ipairs(ids) do
		...
		if t then
			...
			t.description = Untranslated("Loyal to a rival colony. Work suffers when standing is poor. Keep an eye on them.")
			t.group = "Negative"
			t.hidden = false
	...
	for sponsor, title in pairs(RMR_SYMP_NAME) do
		...
		if not TraitPresets[id] then
			PlaceObj("TraitPreset", {
				...
				description = Untranslated("Loyal to a rival colony. Work suffers when standing is poor. Keep an eye on them."),
				group = "Negative",
				hidden = false,
				weight = 0,
				incompatible = {},
